...
WORD_LIST = WORD_LIST.concat(SOLUTION_LIST);
var mot=SOLUTION_LIST[Math.floor(Math.random()*SOLUTION_LIST.length)];
console.log(mot);

//quand la page se charge
window.onload = function() {
    resetGame(); 
    recuperProgressWordle(); 
    scoreActuel = parseInt(localStorage.getItem('scoreActuel')) || 0; // Charger le score depuis localStorage
    afficherScore();
    niveauActuel = parseInt(localStorage.getItem('niveauActuel')) || 0; // Charger le niveau depuis localStorage
    afficherNiveau();
    
    initialiser();  // Cette fonction est appelée ici pour préparer le jeu au chargement de la page
...
function initialiser() 
{
    ...
    for(let r=0;r<clavier.length;r++)       //cree le clavier
    {
        ...
        for(let l=0;l<ligneCourante.length;l++)
        {
            ...
            else if ("a"<=touche && touche<="z")
            {
                ...
                //console.log(document.getElementById('touche-a')); // Devrait afficher l'élément pour la touche 'a'

            }

            toucheClavier.addEventListener("click",gestionTouche);
...
function maj()
{
    ...
    for(let h=0; h<col;h++)       // remplie la ligne de lettre
    {
        ...
        motEntree+=lettreEntree;
    }

    motEntree= motEntree.toLowerCase();     //converti en miniscule
    ...
    {
        let lettreEntree=mot[j];
        if(nblettre[lettreEntree])
        {
    ...
    for(let i=0; i<col; i++)
    {
        ...
        if(correct == col)          //si nombre de toutes les lettres entrees sont correctes et bien place donc 5
        {
            ...
            document.getElementById("reponse").innerText="YOU WIN, CONGRATULATIONS!";
            document.getElementById("btnRejouer").style.display = 'block';
            document.getElementById("btnIndice").style.display = 'none';
            
            scoreActuel += 10;  // Ajouter 10 points pour une victoire
            localStorage.setItem('scoreActuel', scoreActuel); // Sauvegarder le score dans localStorage
            afficherScore(); 

            niveauActuel ++;  // Ajouter 10 points pour une victoire
            localStorage.setItem('niveauActuel', niveauActuel); // Sauvegarder le niveau dans localStorage
            afficherNiveau(); 
            saveProgressWordle(niveauActuel, scoreActuel); 
        }
...
var indiceUtilise = false; // false signifie que l'indice n'a pas encore été utilisé
...
function acheterIndice() {
    ...
    if (indiceUtilise) {
        ...
        return; // Empêche de continuer si l'indice a déjà été utilisé
    }
    
    if (scoreActuel >= 1) { // Vérifier si l'utilisateur a au moins 1 point
        scoreActuel -= 1; // Déduire 1 point
        localStorage.setItem('scoreActuel', scoreActuel); // Mettre à jour le score dans localStorage
        afficherScore(); // Mettre à jour l'affichage du score
        revelerIndice(); // Fonction pour révéler un indice
        
        indiceUtilise = true; // Marquer l'indice comme utilisé
...
function resetGame() {
    // Cette fonction peut être appelée pour réinitialiser le jeu
    indiceUtilise = false; // Réinitialiser l'utilisation de l'indice pour une nouvelle partie
...
function revelerIndice() {
    if (scoreActuel < 1) {
        alert("Vous n'avez pas assez de points pour un indice!");
        return;
    }
    ...
    for (let i = 0; i < mot.length; i++) {
        ...
        // Vérifier chaque tentative précédente pour cette lettre
        ...
        // Si la lettre n'a pas été trouvée dans les tentatives précédentes, révéler comme indice
        if (!lettreDejaTrouvee && !document.getElementById(mot[i]).classList.contains("correcte")) {
            let toucheClavier = document.getElementById(mot[i]); // Accéder à la touche du clavier par l'ID
            if (toucheClavier) {
                toucheClavier.classList.add("malplacee"); // Appliquer le style pour les indices
                indiceRevele = true;
            }
        }
    }
